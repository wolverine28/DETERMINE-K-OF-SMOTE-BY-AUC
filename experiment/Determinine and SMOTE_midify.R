...
determineK <- function(data,class.column){
  ...
  # ROCK <- vector('numeric',0)
  PRK <- vector('numeric',0)
  ...
  for(m in 1:nrow(minor)){
    ...
    # print(which.max(AUC.ROC))
    # print(which.max(AUC.PR))
    
    # ROCK <- c(ROCK,(which.max(AUC.ROC)[1]-1))
    PRK <- c(PRK,(which.max(AUC.PR)-1))
...
doSMOTE <- function(data,class.column,N,SMOTEK){
  ...
  candi <- apply(make,1,function(x){knn$nn.index[x[1],x[2]]})
  
#   for(i in 1:nrow(make))
#     print(knn$nn.index[make[i,1],make[i,2]])
#   
  newset <- matrix(NA,nrow = 0,ncol = ncol(data))
  for(i in 1:(N/100)){
    new <- cbind(minor[make.idx,]+runif(nrow(minor))*(minor[candi,]-minor[make.idx,]),1)
