...
for(s in 1:length(set)){
  ...
  for(m in 1:4){
    write.csv(smote_result_NN[,,m],paste("Z:/개인사용자/윤재섭/DETERMINE K OF SMOTE BY AUC/results/Final/ClassNoise_00%/doSMOTE/",setname,"_OneNN_doSMOTE_",perf[m],".csv",sep=""),row.names=F)
    write.csv(smote_result_CART[,,m],paste("Z:/개인사용자/윤재섭/DETERMINE K OF SMOTE BY AUC/results/Final/ClassNoise_00%/doSMOTE/",setname,"_CART__doSMOTE_",perf[m],".csv",sep=""),row.names=F)
  }
}